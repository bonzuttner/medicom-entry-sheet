...
export default async function handler(req: any, res: any) {
  ...
  if (isAdmin(currentUser)) {
    ...
    // RETAILER: 担当メーカーのシートを取得
    const manufacturerIds = currentUser.assignedManufacturerIds || [];
    if (manufacturerIds.length === 0) {
      totalCount = 0;
      sheets = [];
    } else {
      totalCount = await SheetRepository.countByManufacturerIds(manufacturerIds);
      sheets = hasPagingQuery
        ? await SheetRepository.findByManufacturerIds(manufacturerIds, pageSize + 1, pageOffset)
        : await SheetRepository.findByManufacturerIds(manufacturerIds);
    }
  } else {
    // STAFF: 自社メーカーのシートのみ
